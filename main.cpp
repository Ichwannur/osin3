...
double press=0,pressplus,pressplus1=10+0,pressplus2=0,pressplus3=0,pressplus4=0,pressplus5=0; //tombol pada kipas

bool Toleh=false,Tolehpress=true; //kipas angin bergerak
...
glClearColor(0.95,0.40,0.70,0.0); //BACKGROUND
}
...
glTranslatef(0,-17,-70); // POSISI X,Y TAMPILAN AWAL  untuk menghasilkan terjemahan dari matrix yang ditentukan 3 parameter(x,y,z)
...
cylinder(2.5,1.5,16); // cilinder BATANG BAWAH 2
cylinder(2.5,2.5,6); // cilinder BATANG BAWAH 1
glPushMatrix(); //untuk menyimpan koordinat yang ada
...
if ( Toleh==true)
{
if(Turn >= 60)      // max degrees right ARAH PUTARAN
RightTurn =false;
...
cylinder(4,4,6);// silinder belakang kipas BAGIAN KEPALA KIPAS YANG TERDAPAT TOBOL NAIKNYA
cylinder(1,0.5,15);//silinder tonjolan di depan kipas LINNGKARAN KIPAS YANG BERWARNA PUTIH
glRotatef(270,1.0,0.0,0.0);
if(Tolehpress==true)  // press down turn left-right head button TOMBOL KEPALA KIPAS ---
cylinder(0.3,0.5,6);
else // pull up turn left-right head button
cylinder(0.3,0.5,7);//-- SAMPAI DISINI
glRotatef(90,1.0,0.0,0.0);
glPushMatrix();
glTranslatef(0.0,0.0,11); //POSISI HADAP KIPAS SAAT DI JALANKAN
glutWireTorus(5,7,10,64); //RING DEPAN atau MUKA KIPAS
glutSolidTorus(0.5,12,10,64); //BULATAN/LINGKARAN RING KIPAS
rotation_y+=rotation_y_plus;
if(rotation_y>359)rotation_y=0;
glRotatef(rotation_y,0.0,0.0,1.0);
bilah(3,10,3,5); //  BALING-BALING     bilah(inner radius, outer radius, thickness, qty bilah)
glPopMatrix();
glPopMatrix();
glPopMatrix();
glRotatef(90,1.0,0.0,0.0);
glTranslatef(0.0,-1.0,-4);
blok(2,7,10);// PAPAN BAWAH blok bawah(papan kontrol)
...
blok(0.5,2,2);// untuk blok tombil f1 UNTUK TOMBOL PENCETAN KIPAS
glPopMatrix();
glTranslatef(2.0,0.0,0.0);
glPushMatrix();
...
{
rotation_y_plus=5*direction;//tampilan kecepatan perputaran baling2
...
{
rotation_y_plus=7*direction;//tampilan kecepatan perputaran baling2
...
{
rotation_y_plus=10*direction;//tampilan kecepatan perputaran baling2
speed1=false;//kondisi false
pressplus1=0;//tombol kipas f1 naik
speed2=false; //kondisi false
pressplus2=0; // tobol kipas f2 naik
